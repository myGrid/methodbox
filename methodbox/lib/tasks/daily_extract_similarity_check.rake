#Find out which extracts created in the last day have similar variables and if there are any patterns

require 'rubygems'
require 'rake'

namespace :obesity do
  desc "check any new extracts for similarities and patterns"
  task :daily_extract_similarity_match  => :environment do
    ...
    Csvarchive.all(:conditions=>{:created_at => (Time.now.midnight - 1.day)..Time.now}).each do |extract|
