...
#  else
  ...
  case params['sort']
  when "variable"
    @sorted_variables = @sorted_variables.sort_by { |m| m.name }
  when "description"   then "description"
    @sorted_variables = @sorted_variables.sort_by { |m| m.value }
  when "survey" then "survey"
    @sorted_variables = @sorted_variables.sort_by { |m| Survey.find(m.survey_id).surveytype }
  when "year" then "year"
    @sorted_variables = @sorted_variables.sort_by { |m| Survey.find(m.survey_id).year }
  when "variable_reverse"  then "variable DESC"
    @sorted_variables = @sorted_variables.sort_by { |m| m.name }.reverse
  when "description_reverse"   then "description DESC"
    @sorted_variables = @sorted_variables.sort_by { |m| m.value }.reverse
  when "survey_reverse" then "survey DESC"
    @sorted_variables = @sorted_variables.sort_by { |m| Survey.find(m.survey_id).surveytype }.reverse
