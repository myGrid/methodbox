...
#        flash[:notice] = 'Profile picture was successfully updated.'
  ...
  def check_owner_specified
    ...
    if (@avatar_for.nil? || @avatar_for_id.nil?)
      flash[:error] = "Please use nested routes. Pictures are only available for people, projects and institutions."
  ...
  def find_avatar_auth
    ...
    unless self.action_name.to_s == "show"
      if @avatar_for.downcase == "person"
        ...
        unless @avatar_for_id.to_i == current_user.person.id.to_i || @avatar_owner_instance.can_be_edited_by?(current_user)
          flash[:error] = "You can only view and manage your own pictures, not those of other users."
          redirect_to(person_path(current_user.person))
          return false
        end
      else
        ...
        unless @avatar_owner_instance.can_be_edited_by?(current_user)
          flash[:error] = "You can only view and, possibly, manage pictures of #{pluralize @avatar_for.downcase}, where you are a member of."
    ...
    begin
      ...
      flash[:error] = "Picture not found or belongs to a different #{@avatar_for.downcase}."
      redirect_to(person_path(current_user.person))
      return false
    end
  ...
  def find_avatars
    ...
    if current_user.is_admin? || (@avatar_owner_instance.class.name == "Person" && @avatar_for_id.to_i == current_user.person.id.to_i) ||
      ...
      flash[:error] = "You can only view pictres that belong to you or any projects/institutions where you are a member of."
      redirect_to(person_path(current_user.person))
      return false
    end
  end
