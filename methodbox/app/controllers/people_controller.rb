...
class PeopleController < ApplicationController
  ...
  def index
    ...
    if (!params[:expertise].nil?)
      ...
      @people=Person.tagged_with(@expertise, :on=>:expertise, :conditions => {:dormant => condition})
    elsif (!params[:tools].nil?)
      @tools=params[:tools]
      @people=Person.tagged_with(@tools, :on=>:tools, :conditions => {:dormant => condition})
    elsif (params[:discipline_id])
      @discipline=Discipline.find(params[:discipline_id])
      #FIXME: strips out the disciplines that don't match
      @people=Person.find(:all,:include=>:disciplines,:conditions=>["disciplines.id=?",@discipline.id],:page=>{:size=>default_items_per_page,:current=>params[:page]}, :order=>:last_name, :conditions => {:dormant => condition})
      ...
      @people=Person.find(:all,:include=>[:group_memberships], :order=>:last_name, :conditions => {:dormant => condition})
      ...
      @people = Person.find(:all, :page=>{:size=>default_items_per_page,:current=>params[:page]}, :order=> "last_name, first_name", :conditions => {:dormant => condition})
    end
